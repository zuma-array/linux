 ...
 */
static long clk_int_pll1443x_calc_rate(u32 m, u32 p, u32 s, s16 k, unsigned long parent_rate)
{
 ...
 * m should be in range of a few hundred and p and s are usually below 6.
 */
static s32 clk_int_pll1443x_calc_k(u32 rate, u32 m, u32 p, u32 s, u32 parent_rate)
{
	return DIV_ROUND_CLOSEST(((u64)rate * p * (1 << s) * 65536), parent_rate) - ((s32)m * 65536);
}

static const struct imx_int_pll_rate_table *clk_int_pll1443x_skewable_get_closest(struct clk_int_pll *pll, unsigned long rate)
{
	const struct imx_int_pll_rate_table *rate_table = pll->rate_table;
	int i;
...
static long clk_int_pll1443x_skewable_round_rate(struct clk_hw *hw, unsigned long rate, unsigned long *prate)
{
	struct clk_int_pll *pll = to_clk_int_pll(hw);
	const struct imx_int_pll_rate_table *rate_entry;
	s32 frac;

	rate_entry = clk_int_pll1443x_skewable_get_closest(pll, rate);
	if (rate_entry == NULL)
		return clk_int_pll_round_rate(hw, rate, prate);

	/*
	 * At this point we have a close rate, so we try to recalc the fractional part to get closer
	 * to the requested rate.
	 */
	frac = clk_int_pll1443x_calc_k(rate, rate_entry->mdiv, rate_entry->pdiv, rate_entry->sdiv, *prate);
	if (frac < S16_MIN || frac > S16_MAX) {
		pr_warn("%s: fractional part is outside of range\n", clk_hw_get_name(hw));
		frac = (s16)rate_entry->kdiv;
	}

	return clk_int_pll1443x_calc_rate(rate_entry->mdiv, rate_entry->pdiv, rate_entry->sdiv, frac, *prate);
}
...
static int clk_int_pll1443x_skewable_set_rate(struct clk_hw *hw, unsigned long drate, unsigned long prate)
{
	struct clk_int_pll *pll = to_clk_int_pll(hw);
	const struct imx_int_pll_rate_table *rate_entry;
	...
	rate_entry = clk_int_pll1443x_skewable_get_closest(pll, drate);
	if (rate_entry == NULL)
		return clk_int_pll1443x_set_rate(hw, drate, prate);
	 ...
	 * to the requested rate.
	 */
	frac = clk_int_pll1443x_calc_k(drate, rate_entry->mdiv, rate_entry->pdiv, rate_entry->sdiv, prate);
	if (frac < S16_MIN || frac > S16_MAX) {
		pr_warn("%s: fractional part is outside of range\n", clk_hw_get_name(hw));
		frac = (s16)rate_entry->kdiv;
	}
...
static const struct clk_ops clk_pll1443x_skewable_ops = {
	...
	.is_prepared	= clk_int_pll_is_prepared,
	.recalc_rate	= clk_int_pll1443x_recalc_rate,
	.round_rate	= clk_int_pll1443x_skewable_round_rate,
	.set_rate	= clk_int_pll1443x_skewable_set_rate,
};
