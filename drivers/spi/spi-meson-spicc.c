...
/* This function returns true whenever the configuration has changed, which also
 * includes the transfer speed*/
static bool meson_spicc_setup_xfer(struct meson_spicc_device *spicc,
				   struct spi_transfer *xfer)
{
	u32 conf, conf_orig;
	bool ret;
