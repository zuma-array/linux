...
#define SMCCC_DDR_DVFS_TARGET_1600MTS		0
#define SMCCC_DDR_DVFS_TARGET_667MTS		1
#define SMCCC_DDR_DVFS_WAIT_BUS_FREQ_DONE	0x0F
...
/*
 * From the original busfreq-imx8mq.c implementation.
 */
static irqreturn_t wait_in_wfe_irq(int irq, void *dev_id)
{
	struct arm_smccc_res res;
	/* call smc trap to ATF */
	arm_smccc_smc(FSL_SIP_DDR_DVFS, SMCCC_DDR_DVFS_WAIT_BUS_FREQ_DONE, 0,
		0, 0, 0, 0, 0, &res);
...
/*
 * Based on the original busfreq-imx8mq.c implementation.
 */
static int init_simple_busfreq_irq(struct platform_device *pdev)
{
	...
	for_each_online_cpu(cpu) {
		int irq = platform_get_irq(pdev, cpu);
		if (irq < 0)
			return irq;

		ret = devm_request_irq(dev, irq, wait_in_wfe_irq, IRQF_PERCPU, "ddrc", NULL);
		if (ret)
			return ret;
...
static void set_bus_freq(struct simple_busfreq_data *pdata, bool high)
{
	...
	if (high) {
		update_ddr_freq(SMCCC_DDR_DVFS_TARGET_1600MTS);
		...
		update_ddr_freq(SMCCC_DDR_DVFS_TARGET_667MTS);
...
#undef CLK_GET
	...
	ret = init_simple_busfreq_irq(pdev);
	if (ret) {
		dev_err(dev, "Failed to setup busfreq interrupts.\n");
		return ret;
	}
	...
	ret = device_create_file(dev, &dev_attr_set_high);
	if (ret) {
...
static int simple_busfreq_remove(struct platform_device *pdev)
{
	...
	set_bus_freq(pdata, true);
	device_remove_file(dev, &dev_attr_set_high);
