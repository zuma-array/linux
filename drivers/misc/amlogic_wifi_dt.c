...
static int wifi_dev_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		if (ret) {
			...
			plat->interrupt_pin = of_get_named_gpio_flags(pdev->dev.of_node,
		...
		if (ret) {
			...
			wifi_power_gpio = 1;
			plat->power_on_pin = of_get_named_gpio_flags(pdev->dev.of_node,
			"power_on_pin", 0, NULL);
			if (plat->power_on_pin < 0) {
				return plat->power_on_pin;
			}
		}
		...
		if (ret) {
			...
			wifi_power_gpio2 = 1;
			desc = gpio_to_desc(of_get_named_gpio_flags(pdev->dev.of_node,
