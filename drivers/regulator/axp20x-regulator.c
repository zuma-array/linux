...
static const struct linear_range axp152_ldo0_ranges[] = {
	REGULATOR_LINEAR_RANGE(2500000, 0x03, 0x03, 0x00),
	REGULATOR_LINEAR_RANGE(2800000, 0x02, 0x02, 0x00),
	REGULATOR_LINEAR_RANGE(3300000, 0x01, 0x01, 0x00),
	REGULATOR_LINEAR_RANGE(5000000, 0x00, 0x00, 0x00),
};
...
static const struct regulator_desc axp152_regulators[] = {
	...
	AXP_DESC_RANGES(AXP152, LDO0, "ldo0", "ldo0in", axp152_ldo0_ranges,
			4, AXP152_LDO0_CTRL, 0x30, AXP152_LDO0_CTRL, BIT(7)),
...
static int axp20x_suspend(struct device *dev)
{
	...
	for (i = 0; i < pdata->nregulators; i++) {
		...
		regulator_lock(rdev);
		if (rdev->constraints->state_mem.enabled == DISABLE_IN_SUSPEND 
